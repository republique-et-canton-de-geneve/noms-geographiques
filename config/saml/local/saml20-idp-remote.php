@@ -1,203 +0,0 @@
<?php

$metadata['***REMOVED***:8080***REMOVED***/metadata.php'] = [
  'metadata-set' => 'saml20-idp-remote',
  'entityid' => '***REMOVED***:8080***REMOVED***/metadata.php',
  'SingleSignOnService' => [
    [
      'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
      'Location' => '***REMOVED***:8080***REMOVED***/SSOService.php',
    ],
  ],
//  'SingleLogoutService' => [
//    [
//      'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
//      'Location' => '***REMOVED***:8080***REMOVED***/SingleLogoutService.php',
//    ],
//  ],
  'certData' => '',
  'NameIDFormat' => 'urn:oasis:names:tc:SAML:2.0:nameid-format:transient',
  'contacts' => [
    [
      'emailAddress' => 'na@example.com',
      'contactType' => 'technical',
      'givenName' => 'Administrator',
    ],
  ],
];
