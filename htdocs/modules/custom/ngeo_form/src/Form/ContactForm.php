...
class ContactForm extends FormBase {
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if (!$form_state->hasValue('message')) {
      $form_state->setErrorByName('message', $this->t("Le message est requis."));
    }
    ...
    if (mb_strlen($form_state->getValue('message')) < 10) {
      $form_state->setErrorByName('message', $this->t("Le message est trop court."));
      return;
    }

    if ($this->ngeoFormHelper->isSpam($form_state->hasValue('message'))) {
      $this->logger("ngeo_form")->info("Contact - SPAM detected");
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->messenger()->addStatus($this->t('Le message a bien été envoyé, merci.'));
  }
  ...
  private function sendNotificationMail($data) {
    ...
    $params = [
      'message' => '',
      'subject' => t('Noms géographiques - Contact'),
    ];
    ...
    $params['message'] .= "\n";
    $params['message'] .= "Message : " . $data['message'] . "\n";
    $params['message'] .= "\n";

    $params['message'] .= "Soumis à : " . (new \DateTime())->format('Y-m-d H:i:s') . "\n";
    $langcode = \Drupal::currentUser()->getPreferredLangcode();

    $this->mailManager->mail('ngeo_form', 'sendContactMail', $to, $langcode, $params, NULL, TRUE);
  }
